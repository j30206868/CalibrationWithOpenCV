...
std::string get_full_filename(int file_num, const char *path, const char *filename, const char *suffix){
	stringstream sstm;
	if(file_num < 10)
		sstm << path << filename << 0 << file_num << suffix;
	else
		sstm << path << filename << file_num << suffix;
	return sstm.str();
}
...
int do_single_calibration(){
    ...
    help();
    Settings s;
    const string inputSettingsFile = "single_calibration/right_single_calib.xml";
    ...
    if( s.inputType == Settings::IMAGE_LIST && s.showUndistorsed )
    {
        ...
        for(int i = 0; i < (int)s.imageList.size(); i++ )
        {
			...
			imwrite( get_full_filename(i+1, "calibrated/", s.calibPart.c_str(), ".bmp"), rview );
